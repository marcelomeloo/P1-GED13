...
```{r Grafico de linha do Preco de Fechamento DJI}
...
```{r Retorno, com base no Preco de Fechamento DJI}
...
```{r Grafico de linha do Retorno DJI}
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados DJI}
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados DJI}
...
```{R QQLine do retorno (fazer junto com o QQPlot). DJI}
...
```{R Assimetria amostral n?o viesada do retorno. DJI}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral n?o viesada do retorno. DJI}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){

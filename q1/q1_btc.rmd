...
```{r Grafico de linha do Preco de Fechamento BTC}
...
```{r Retorno, com base no Preco de Fechamento BTC}
...
```{r Grafico de linha do Retorno BTC}
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados BTC}
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados BTC}
...
```{R QQLine do retorno (fazer junto com o QQPlot). BTC}
...
```{R Assimetria amostral nao viesada do retorno.  BTC}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral nao viesada do retorno. BTC}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){

...
```{r Grafico de linha do Preco de Fechamento IXIC}
...
```{r Retorno, com base no Preco de Fechamento IXIC}
...
```{r Grafico de linha do Retorno IXIC}
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados IXIC}
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados IXIC}
...
```{R  QQLine do retorno (fazer junto com o QQPlot). IXIC}
...
```{R Assimetria amostral nao viesada do retorno. IXIC}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral nao viesada do retorno. IXIC}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){

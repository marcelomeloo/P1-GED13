...
```{r Grafico de linha do Preco de Fechamento USD}
...
```{r Retorno, com base no Preco de Fechamento USD}
...
```{r Grafico de linha do Retorno USD}
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados USD}
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados USD}
...
```{R QQLine do retorno (fazer junto com o QQPlot). USD}
...
```{R Assimetria amostral nao viesada do retorno. USD}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral nao viesada do retorno. USD}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){

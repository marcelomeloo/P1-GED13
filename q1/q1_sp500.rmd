...
```{r Grafico de linha do Preco de Fechamento}
...
```{r Retorno, com base no Preco de Fechamento }
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados }
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados }
...
```{R QQLine do retorno (fazer junto com o QQPlot)}
...
```{R Assimetria amostral nao viesada do retorno.}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral nao viesada do retorno}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){

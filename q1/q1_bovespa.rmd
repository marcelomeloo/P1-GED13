...
```{r Grafico de linha do Preco de Fechamento BVSP}
...
```{r Retorno, com base no Preco de Fechamento BVSP}
...
```{r Grafico de linha do Retorno BVSP}
...
```{R Box plot para dados originais (Preco de Fechamento e Retorno) e padronizados BVSP}
...
```{R Histograma para dados originais (Preco de Fechamento e Retorno) e padronizados BVSP}
...
```{R QQLine do retorno (fazer junto com o QQPlot). BVSP}
...
```{R Assimetria amostral nao viesada do retorno. BVSP}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
...
```{R Curtose amostral nao viesada do retorno BVSP}
  n <- length(retorno_pf)
  somatorio <- c()
  for(i in 1:n){
